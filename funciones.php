<?php
	...
	function usuarioExiste($usuario)
	{
		...
		$stmt = $conn->prepare("SELECT id FROM login WHERE user = ? LIMIT 1");
	...
	function lastSession($id)
	{
		...
		$stmt = $conn->prepare("UPDATE usuarios SET last_session=NOW(), token_password='', password_request=1 WHERE id = ?");
		$stmt->bind_param('s', $id);
		$stmt->execute();
		$stmt->close();
	}
